 ...
 */
@SpringBootTest class BookRepositoryTest {
    private static final UUID AUTHOR_ID = UUID.fromString("03874eac-3be2-4814-91b8-e4baaf8321eb");
    private static final UUID BOOK_ID = UUID.fromString("add9ec48-9b41-4e2e-9e9e-120d3459df02");
    private final BookRepository bookRepository;
    private final AuthorRepository authorRepository;

    @Autowired BookRepositoryTest(BookRepository bookRepository, AuthorRepository authorRepository) {
        this.bookRepository = bookRepository;
        this.authorRepository = authorRepository;
    }

    @Test void SaveCascadeTest() {
        Author author = new Author("Jane doe", LocalDate.of(1967, 9, 12), "United States");
        authorRepository.save(author);

        Book book = new Book("50827-14304",
                             "The Greatest Book",
    ...
    @Transactional @Test void findBook() {
        Book book = bookRepository.findById(BOOK_ID).orElseThrow();
        System.out.println("Book: " + book.getId());
        System.out.println("Author: " + book.getAuthor().getId());
    }

    @Test void delete() { bookRepository.deleteById(BOOK_ID); }

    @Test void modifyAuthorOfBook() {
